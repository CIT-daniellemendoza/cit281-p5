...
//function to list out monsters from a list
function addMonsterToList(item) {
  const {monsterName} = item
...
module.exports = class MonsterGame {
  ...
  // List monsters
  /*
  Started to work on this function, but can only get it to console.log each monsters name, 
  not in list format as shown in your example output code, not sure if this still works for what you want though

  Cannot get it to work within all of these files though, this code when put into a file by itself and run works (lines 39 through 45): 
